/**
 * Vinyl committee selector — mirrors ecosoc / unsc flow.
 * On Enter for current committee (Press Corps), hides wheel and shows payment gate.
 */
(function () {
  ...
  function logoUrl(file) {
    return "assets/committee-logos/" + file + "?v=" + LOGO_VER;
  }
  ...
  function render() {
    ...
    var logoSrc = c.logo ? logoUrl(c.logo) : "";
    ...
    var labels = COMMITTEES.map(function (item, i) {
      ...
      var fill = isSelected ? "rgb(191 219 254)" : "rgba(255,255,255,0.6)";
      var dot =
        isSelected
          ? '<circle cx="' +
            ...
            '" r="4" fill="#60a5fa" style="filter:drop-shadow(0 0 2px rgba(0,0,0,0.5))"></circle>'
          : "";
      return (
    ...
    root.innerHTML =
      ...
      (isCurrent ? "Continue into Press Corps" : "Continue to selected committee") +
      '" title="' +
      ...
      "</button>" +
      (isCurrent
        ? '<p class="wheel-hint">Press Corps committee portal — topic, allocations, resources</p>'
        : "") +
      "</div>" +
      '<a class="wheel-seamun" href="https://www.seamun.com" target="_blank" rel="noopener noreferrer">seamun.com</a>' +
      "</div>";
  ...
  function enter() {
    ...
    if (c.isCurrent) {
      ...
      if (paymentGate) {
        ...
        window.dispatchEvent(new CustomEvent("seamun:show-payment"));
      } else if (siteApp) {
  ...
  function bind() {
    ...
    wheelEl.addEventListener("pointermove", function (e) {
      ...
      if (isDragging && rotator) {
        ...
        if (img && dc.logo) img.src = logoUrl(dc.logo);
